...
export default function Header({ onNavigate, activeSection }: HeaderProps) {
  ...
  // Lock body scroll when mobile menu is open to keep background stable and non-interactive
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <>
      {/* Mobile Backdrop - Blurs and darkens page background when mobile menu is open */}
      <AnimatePresence>
        {isOpen && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 bg-charcoal/40 backdrop-blur-xs z-40 md:hidden"
      ...
      <header
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled || isOpen
            ? "bg-warm-cream shadow-sm border-b border-warm-beige/60 py-3"
            : "bg-transparent py-5"
        }`}
      >
            ...
            >
              <img 
                ...
                className={`w-auto object-contain transition-all duration-300 ${
                  scrolled || isOpen ? "h-20 md:h-24" : "h-32 md:h-40"
                }`}
                onError={(e) => {
